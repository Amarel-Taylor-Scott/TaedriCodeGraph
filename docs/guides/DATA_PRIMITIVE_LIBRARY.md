...
The checked-in registry evidence contains 11 active releases: two text primitives and
nine new data-cleaning, data-engineering, and data-science primitives. Each can be
...
All nine have explicit error behavior, limitations, positive/boundary/negative examples,
...
SQLite database has 11 active releases and 109 content-addressed blobs. The run executed
66 cases, served 11 targeted searches with exactly one intended result each, and found
18 directional compatibility edges by schema-digest blocking before exact assessment.

It then downloaded the release packs and executed two routes without a model call or
generated glue:
...
1. explicit missing-value normalization with a declared sentinel policy;
2. locale-independent numeric coercion with per-item failure receipts;
3. record projection and rename maps with collision rules;
4. deterministic sort/group/window operations with stability and memory contracts;
5. typed joins with null, key-cardinality, and duplicate-key policies;
6. quantiles, robust scaling, covariance, correlation, and evaluation metrics;
7. streaming and Arrow-native variants for inputs too large to materialize;
8. dependency-backed pandas, Polars, PyArrow, NumPy, and scikit-learn adapters after the
